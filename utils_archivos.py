...
def solicitar_semana(tipo: str = "proyecto") -> str:
    ...
    if semana_env:
        ...
        if semana.startswith('S') and semana[1:].isdigit():
            print(f"📌 Semana desde variable de entorno: {semana}")
            return semana
    
    prompt = f"📅 Ingresa el número de semana (ej: S183): "
...
def mostrar_resumen_archivos(rutas: dict, output_dir: Path, semana: str) -> None:
    ...
    if 'dashboard_actual' in rutas:
        print(f"✅ Dashboard: dashboards/{rutas['dashboard_actual'].name}")
    
    if 'tabla_resumen' in rutas:
        print(f"✅ Tabla Resumen: tablas/{rutas['tabla_resumen'].name}")
    ...
    print(f"  output/")
    print(f"    ├── dashboards/     ← Dashboards interactivos")
    print(f"    ├── tablas/         ← Tablas IBCS")
    print(f"    └── historico/      ← Archivos por semana de proyecto")
    print(f"  data/")
    print(f"    └── historico/")
    print(f"         └── consolidado/  ← Respaldos Excel por semana")
    print(f"{'='*60}\n")
